"""Core geometry and dimension helpers for the Koch Snowflake mini-app.

This file is intentionally DOM-free so it can be tested in normal CPython and
reused by a Pyodide/PyScript browser bridge.
"""
...
def normalize_points(points, width, height, padding=24):
    ...
    for x, y in points:
        ...
        normalized.append((sx, height - sy))
    return normalized


def koch_curve(p1, p2, depth, randomness, rng):
    """Return points for one recursive Koch segment.

    Randomness is bounded to the height of the triangular bump. The endpoints
    and thirds remain stable, so the curve keeps its recursive structure while
    gaining organic irregularity.
    """
    if depth == 0:
        return [p1, p2]
...
def box_count_dimension(points, box_sizes=(8, 16, 32, 64, 128)):
    ...
    numerator = sum((x - x_mean) * (y - y_mean) for x, y in zip(log_eps, log_n))
    denominator = sum((x - x_mean) ** 2 for x in log_eps)

    if denominator == 0:
        return None, log_eps, log_n, None, None

    # log(N) ≈ intercept + slope * log(1 / epsilon)
